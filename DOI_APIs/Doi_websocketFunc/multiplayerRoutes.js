...
const multiplayerBoard = async ({reqData , ws , gameConnectionDB , userConnectionDB}) =>{
    switch(reqData.post)
    {
        case 'connection':
            if(reqData.body)
        ...
        case 'playerGuess':
            if(reqData.body)
            {
                ...
                if(jwtToken && gameID && guess && clockTimer && gameMode)
                {
                    ...
                    {
                        reqData.body.userID = payload.cred;
                        if(gameMode === multiplayerGameModes.mode1) await getCallsNupdate_MODE1(reqData.body , ws , gameConnectionDB);
...
module.exports = {
    multiplayerRoute:multiplayerBoard
}