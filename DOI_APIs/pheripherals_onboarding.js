const { createUserStats } = require("./utils/createUserUtils");
...
const userRegistration = async ({connection , initID , cred , userStatsConnection , leaderBoardConnection}) =>{
    try
    {
        const userExists = await connection.findOne({
        ...
        if(!userExists)
        {
            ...
            const userAccount = new connection({userID:userID , username:cred.username , country:cred.country , avatar:cred.avatar , deviceID:cred.deviceID});

            await userAccount.save();


            const statsObject = new createUserStats({userID , username:cred.username , country:cred.country})

            const userStats = new userStatsConnection(statsObject);

            await userStats.save();


            const userLeaderBoard = new leaderBoardConnection({username:cred.username , userID , country:cred.country , XP:0})
