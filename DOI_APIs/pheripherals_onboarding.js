    ...
    try{
        ...
        if(userExists)
        {
            if(cred.password === userExists.password)
            {
                ...
                if(newDeviceUser) return {ok:true , data:{type:"loginSuccessful" , msg:createJwtToken({cred:userExists.userID})}};
                else{
...
const userRegistration = async ({connection , initID , cred}) =>{
    try
    {
        ...
        if(!userExists)
        {
            ...
            return {ok:true , data:{type:"registrationsuccess" , msg:createJwtToken({cred:userID})}}
...
const deviceIDAuth = async({connection , cred}) =>{
    try
    {
        ...
        if(userExists)
        {
            return {ok:true , data:{type:"IDAuthSuccess" , msg:createJwtToken({cred:userExists.userID})}}
