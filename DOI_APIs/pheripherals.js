    ...
    try{
        ...
        if(userExists)
        {
            if(cred.password === userExists.password)
            {
                ...
                if(newDeviceUser) return {ok:true , data:{type:"loginSuccessful" , msg:userExists.userID}};
                else{
...
const deviceIDAuth = async({connection , cred}) =>{
    try
    {
        ...
        if(userExists)
        {
            return {ok:true , data:{type:"IDAuthSuccess" , msg:userExists.userID}}
    ...
    try{
        ...
        if(userExists)
        {
            if(!userExists.username.includes(cred.username))
            {
                ...
                if(addUserName_) return {ok:true , data:{type:"nameAddSuccessful" , msg:userExists.userID}};
    ...
    try{
        ...
        if(userExists)
        {
            if(userExists.avatar !== cred.avatar)
            {
                ...
                if(updateAvatar_) return {ok:true , data:{type:"avatarUpdateSuccessful" , msg:"Username updated successfully"}};
